...
%TotalTime = 6000;
TotalTime = 6000;
%%%%% Computing the region of the period of c4 such that x4 is in spiking mode %%%%%%%%%
memoryLen = params4(1);
activeThreshold = params4(2) ;
period3 = params3(3);
endPeriod = floor(period3*2/3) - 1;
startPeriod = 2; 
step_period = 5;
...
%% Generating data and store in Data_inp_exp %%%

for k = 2:num_period
    k
    signal_period = startPeriod + step_period*(k-1);
    
    params4(3) = signal_period;
    ...
    for i = 1:10
        ...
        %% The following is for the purpose of checking the time series. Comment them out if you don't want see the figures.
        node1 = Net1(:,1);
...
%% Rewrite the data in a matrix form in order to be ready for plot in R.
...
save('AllData_varyingc4_p3_121.mat', 'All_data')
